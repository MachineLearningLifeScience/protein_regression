...
for dataset in datasets:
    ...
    for repr in simons_algos.keys():
        for a in simons_algos[repr]:
            ...
            assert(len(exps) == 1)
            if a.get_name() not in algorithm_name_list:
            ...
            result_dict[a.get_name()] = results
    results_dict[dataset] = result_dict

# Richard's code
print(results_dict)
# then calls #plot_metric_for_dataset
plot_metric_for_dataset(datasets=datasets, metric_values=results_dict, reps=algorithm_name_list, cvtype="cv split")
