...
class MLPEnsemble(AbstractAlgorithm):
    def __init__(self, optimize: bool=False, opt_budget: int=10, seed=10, ensemble_size=3) -> None:
        self.model = None
    ...
    def train(self, X: np.ndarray, Y: np.ndarray) -> None:
        assert(Y.shape[1] == 1)
        self.model = [MLPRegressor(random_state=self.seed+_e) for _e in range(self.ensemble_size)]
    ...
    def predict(self, X: np.ndarray):
        ...
        """
        predictions = np.array([model.predict(X).reshape(-1, 1) for model in self.model])
