...
class GMMRegression(AbstractAlgorithm):
    ...
    def train(self, X, Y):
        prior_assignments = np.concatenate(
            [
                np.array(Y < self.threshold).astype(np.float64),
                np.array(Y >= self.threshold).astype(np.float64),
            ],
            axis=1,
        )
        self.model = GaussianMixtureRegressor(
            n_components=self.n_components, init_params="kmeans++"
        )
    ...
    def predict(self, X):
        ...
        cluster_vec = np.argmax(
            self.model.gmm_.to_responsibilities(np.hstack([X, pred])), axis=-1
        )
        sigma_2 = []
        for cluster in cluster_vec:
            # NOTE: computing uncertainty sigma relative to Gaussian of MM: weight * (tr(\sigma)/2)
            _sigma = (
                np.trace(self.model.gmm_.covariances[cluster]) / self.n_components
            ) * self.model.gmm_.priors[cluster]
