...
def extract_parameters(
    ...
    results_dict = get_mlflow_results(
        datasets, algos, reps, metrics, train_test_splitter
    )
    cv_name = train_test_splitter(datasets[0]).get_name()
    sub_metrics = ["mu", "std"]
    header_multi_index = pd.MultiIndex.from_tuples(
        (
            (a, m, sm)
            for a in results_dict.get(datasets[0]).keys()
            for m in metrics
    ...
    multi_index = pd.MultiIndex.from_tuples(
        ((d, r, cv_name) for d in datasets for r in reps),
    ...
    for alg in results_dict.get(datasets[0]).keys():
        for metric in metrics:
            for dat in datasets:
                for rep in reps:
                    entry = (
                        results_dict.get(dat).get(alg).get(rep).get(None).get(metric)
                    )
                    df.loc[(dat, rep, cv_name), (alg, metric)] = (
                        np.round(np.mean(entry), 2),
                        np.round(np.std(entry), 2),
                    )
    return df


def extract_cv_information(
    datasets: list, representations: list, protocols: list, cached_results=True
) -> pd.DataFrame:
    ...
    if cached_results and os.path.exists(cached_filename):
        ...
        for protocol in protocols:
            mlflow_protocol_results = get_mlflow_results_artifacts(
                datasets=datasets,
                algos=algos,
    ...
    header_multi_index = pd.MultiIndex.from_tuples(
        (d, r) for d, r in product(datasets, representations)
    )
    row_multi_index = pd.MultiIndex.from_tuples(
        (p, idx) for p, idx in product(protocol_names, [0])
    )
...
if __name__ == "__main__":
    ...
    representations = [
        ONE_HOT,
        EVE_DENSITY,
    ]  # splits are the same for ESM/ProtBert/OH , different for EVE derivative
    train_test_splitter = [
        RandomSplitter("1FQG"),
        PositionSplitter("1FQG"),
    ]  # NOTE: fractional splitter is not included here
    cv_df = extract_cv_information(
        datasets=datasets,
        representations=representations,
        protocols=train_test_splitter,
    )
    # main overview
