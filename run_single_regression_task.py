...
def _dim_reduce_X(
    dim: int, dim_reduction: str, X_train: np.ndarray, Y_train: np.ndarray
...
def _run_regression_at_split(
    ...
    if dim and X_train.shape[1] > dim:
        X_train, reducer = _dim_reduce_X(
            dim=dim, dim_reduction=dim_reduction, X_train=X_train, Y_train=scaled_y
        )
        X_test = reducer.transform(X_test).astype(np.float64)
        mlflow.set_tags({"DIM_REDUCTION": dim_reduction, "DIM": reducer.n_components})
    # SET PRIORS IF GP
    if (
        "GP" in method.get_name() and not dim and X_train.shape[0] > 1
    ):  # set initial parameters based on distance in space if using full latent space
        init_len = np.max(np.abs(np.subtract(X_train[0], X_train[1])))
        eps = 0.001
        method.init_length = (
            init_len if init_len > 0.0 else init_len + eps
        )  # if reduced on lower dim this value is too small
    if "GP" in method.get_name():
        ...
        _, prior_cov = (
            np.zeros(X_train.shape[0])[:, np.newaxis],
            np.ones(X_train.shape[0])[:, np.newaxis],
        )
    if "RF" in method.get_name():  # write and load optimal parameters
        method._persist_id = (
            f"{dataset}_{representation}_{protocol.get_name()}_{augmentation}_s{split}"
        )
    # TRAIN MODEL:
    ...
    try:
        ...
        warnings.warn(
            f"Experiment: {dataset}, rep: {representation} in d={dim} not stable, prediction failed!"
        )
    ...
    try:
        ...
        warnings.warn(
            f"Experiment: {dataset}, {representation} at d={dim} posterior is NOT COMPUTABLE!"
        )
    ...
    assert mu.shape == Y_test.shape, (
        "shape mismatch " + str(mu.shape) + " " + str(Y_test.flatten().shape)
    )
    # COMPUTE RESULT METRICS
    baseline = np.mean(
        np.square(Y_test - np.repeat(mean_y, len(Y_test)).reshape(-1, 1))
    )
    ...
    pac_epsilon = alquier_bounded_regression(
        post_mu=np.zeros(n),
        post_cov=unscaled_posterior_cov,
        prior_mu=np.zeros(n),
        prior_cov=unscaled_prior_cov,
        n=n,
        delta=0.05,
        loss_bound=(0, 1),
    )
    ...
    record_dict = {
        ...
        "unc": uncs,
        "mse": errs,
        "train_mutation": train_mutations,
        "test_mutation": test_mutations,
    ...
    if "GP" in method.get_name():
        ...
        mlflow.log_metric(
            GP_L_VAR, float(method.gp.likelihood.variance.numpy()), step=split
        )
        if method.gp.kernel.__class__ != kernels.linears.Linear:
            mlflow.log_metric(
                GP_LEN, float(method.gp.kernel.lengthscales.numpy()), step=split
            )
...
def run_single_regression_task(
    dataset: str,
    representation: str,
    method_key: str,
    protocol: AbstractTrainTestSplitter,
    augmentation: str,
    dim: int = None,
    ...
    method = ALGORITHM_REGISTRY[method_key](
        representation, get_alphabet(dataset), optimize
    )
    print(
        f"{dataset}: {representation} - {method_key} | {protocol.get_name()} [dim: {dim}, opt: {optimize}]"
    )
    ...
    if type(protocol) in [PositionSplitter, BioSplitter]:
        train_indices, _, test_indices = protocol.split(
            X, (representation, augmentation), missed_assay_indices
        )
    ...
    print(
        f" |S|_train={'| '.join([str(len(s)).rjust(5) for s in train_indices])}\n |S|_test ={'| '.join([str(len(s)).rjust(5) for s in test_indices])}"
    )
    if mock:
        return
    ...
    def _run_regression_at_index_function_wrapper(idx: int):
        ...
        """
        return _run_regression_at_split(
            X=X,
            Y=Y,
    ...
    with mlflow.start_run() as run:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=1
        ) as executor:  # NOTE: the function, especially tensorflow optimize is NOT THREADSAFE max_workers=1 required atm.
            tqdm(
                executor.map(
                    _run_regression_at_index_function_wrapper,
