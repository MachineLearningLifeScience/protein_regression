...
def run_single_optimization_task(
    dataset: str,
    method_key: str,
    seed: int,
    representation: str,
    ...
    for i in tqdm(range(1, max_iterations + 1)):
        ...
        if len(selected_X) > 1:
            mean_y, std_y, Y_observed = scale_observations(
                Y_observed
            )  # outputs scaled_y as Y_observed
        method.train(X_observed, Y_observed)
        remaining_X = np.setdiff1d(
            np.arange(X.shape[0]), selected_X, assume_unique=True
        )
        ...
        if i in log_interval:
            _log_optimization_metrics_to_mlflow(
                method=method,
...
def _log_optimization_metrics_to_mlflow(
    method, remaining_Y, mean_y, std_y, _mu, _unc, step
) -> None:
    """
    ...
    # compute metrics
    baseline = np.mean(
        np.square(remaining_Y - np.repeat(mean_y, len(remaining_Y)).reshape(-1, 1))
    )
    ...
    if "GP" in method.get_name():
        ...
        mlflow.log_metric(
            GP_L_VAR, float(method.gp.likelihood.variance.numpy()), step=step
        )
        if method.gp.kernel.__class__ != kernels.linears.Linear:
            mlflow.log_metric(
                GP_LEN, float(method.gp.kernel.lengthscales.numpy()), step=step
            )
    ...
    mlflow.log_dict(
        {"trues": trues, "pred": mus, "unc": uncs, "mse": errs},
        "split" + str(step) + "/output.json",
    )


def run_ranked_reference_task(
    dataset, max_iterations=500, log_interval=1, seed=42, reference_task=EVE_DENSITY
):
    """
