...
def plot_metric_for_mixtures(results: dict, threshold:float, protocol: str, method=GMMRegression(), augmentation=None, split=0) -> None:
    ...
    fig, ax = plt.subplots(3*len(representations), len(dimensions), figsize=(25, 25), gridspec_kw={'height_ratios': [2, 1, 0.5]*len(representations)}) # for each representation two rows
    ...
    for i, d in enumerate(dimensions):
        for j, rep in enumerate(representations):
            ...
            ax[j,i].scatter(true_observations, predictions, color=color_assignment, alpha=0.7)
            ...
            ax[j,i].vlines(threshold, ymin=0, ymax=2., colors="k", linestyles="dashdot", label=f"threshold")
            ax[j,i].hlines(threshold, xmin=0, xmax=2., colors="k", linestyles="dashdot", label=f"threshold")
            ax[j+2,i].scatter(train_X[:, 0], train_X[:, 1], alpha=0.025, label=f"train")
            for c in range(method.n_components):
                ...
                ax[j,i].plot(mean, mean, c=marker_colors[c], marker="o", label=f"mu_{str(c)}")
                ax[j,i].plot(mean*gm_weights[c], mean*gm_weights[c], color=marker_colors[c], marker="x", label=f"w*mu_{str(c)}")
                ax[j,i].plot([mean-sigma, mean+sigma], [mean, mean], color=marker_colors[c], label="var(y)")
                ax[j,i].plot([mean, mean], [mean-sigma, mean+sigma], color=marker_colors[c], label="var(y)")
                ax[j,i].plot([(mean-sigma)*gm_weights[c], (mean+sigma)*gm_weights[c]], [mean*gm_weights[c], mean*gm_weights[c]], color=marker_colors[c], linestyle="dashed", label="w*var(y)")
                xx = np.linspace(mean-2*sigma, mean+2*sigma, 100)
                performance_g0 = mean_squared_error(true_observations[pred_assignments==0], pred_comp0) if len(pred_comp0) > 0 else -1
                performance_g1 = mean_squared_error(true_observations[pred_assignments==1], pred_comp1) if len(pred_comp1) > 0 else -1
                ax[j+1,i].plot([0, 1], [performance_g0, performance_g1], "ko")
                ax[j+2,i].scatter(gm_means[c][0], gm_means[c][1], s=100, c="red", marker="x", label=c)
    ...
    plt.tight_layout()
    plt.savefig(f"./results/figures/mixtures/{dataset}_{protocol}_gmm_visualization.png", bbox_inches='tight')
    plt.savefig(f"./results/figures/mixtures/{dataset}_{protocol}_gmm_visualization.pdf", bbox_inches='tight')
    plt.show()


def __filter_results_given_threshold(results: dict, t: float, metrics: list, observation_id='trues') -> dict:
    """
    ...
    for split in results.keys():
        ...
        filtered_results[split][observation_id] = np.array(results[split].get(observation_id))[idx]
        for metric in metrics:
            filtered_results[split][metric] = np.array(results[split].get(metric))[idx]
    return filtered_results


def __compute_metric_from_filtered_results(filtered_results_array: list, metric_name: str) -> list:
    result_metric = []
...
def plot_metric_against_threshold(results: dict, metrics:list, protocol_name: str):
    ...
    fig, ax = plt.subplots(len(metrics), len(representations), figsize=(25, 15)) # for each representation a column
    ...
    for i, metric in enumerate(metrics):
        for j, representation in enumerate(representations):
            for k, method in enumerate(methods):
                _results = results.get(dataset).get(method).get(representation).get(None)
                observed_min = min([min(_results.get(i_cv).get('trues')) for i_cv in _results]) # GET MIN MAX FROM OBSERVATIONS
                observed_max = max([max(_results.get(i_cv).get('trues')) for i_cv in _results])
                observation_space = np.linspace(observed_min, observed_max, 100)
                n_splits = len(_results.keys())
                reported_metric = np.mean([_results.get(i).get(metric) for i in range(n_splits)])
                ax[i,j].hlines(y=reported_metric, xmin=observed_min, xmax=observed_max, linestyles="dashed", colors=cmap(k), label=f"mean {metric} {method}")
                filtered_observations = [__filter_results_given_threshold(results=_results, t=t, metrics=['pred', 'mse']) for t in observation_space]
                n_results = [len(result.get(0).get('trues')) for result in filtered_observations]
                #filtered_metric = __compute_metric_from_filtered_results(filtered_observations, metric_name=metric)
                if metric == MSE:
                    filtered_metric = [np.mean([np.mean(split_result.get(metric.lower())) for split_result in filtered_result.values()]) for filtered_result in filtered_observations]
                elif metric == SPEARMAN_RHO:
                    filtered_metric = [np.mean([spearmanr(split_result.get('trues'), split_result.get('pred')) for split_result in filtered_result.values()]) for filtered_result in filtered_observations]
                else:
                    raise NotImplementedError(f"Specified metric {metric} not implemented!")
                ax[i,j].plot(observation_space, filtered_metric, c=cmap(k), label=method)
            ...
            labels = [f"{np.round(t,1)} \n {n}" for t, n in zip(observation_space[::10], n_results[::10])]
    ...
    plt.savefig(f"./results/figures/relative_threshold/{dataset}_{protocol_name}_{metrics}_against_threshold.png")
    plt.show()
