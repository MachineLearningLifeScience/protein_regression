...
def barplot_metric_comparison(metric_values: dict, cvtype: str, augmentation: None):
    c = ['dimgrey', '#661100', '#332288', '#117733']
    cc = ['yellow', 'darkgreen', 'orange', 'pink']
    plot_heading = f'Comparison of algoritms and representations, cv-type: {cvtype}'
    filename = 'results/figures/'+'accuracy_of_methods_barplot_'+cvtype+"__"+augmentation
    plot_heading = plot_heading + f", Augmentation: {augmentation}" if augmentation else plot_heading 
        ...
        for i, algo in enumerate(metric_values[dataset_key].keys()):
            ...
            for j, rep in enumerate(metric_values[dataset_key][algo].keys()):
                ...
                if augmentation is not None:
                    axs[d].barh(i+seps[j], neg_invert_mse, xerr=error_on_mean, height=height, label=rep, color=c[j], 
                            facecolor=c[j], edgecolor=cc[j], ecolor='black', capsize=5, hatch='//')
