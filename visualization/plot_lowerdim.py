...
def plot_lower_dim_results(
    datasets: List[str],
    algorithms: List[str],
    representations: List[str],
    cv_types: List[str],
    dimensions: List[int] = [2, 10, 100, 1000, None],
    metrics: List[str] = [MSE, SPEARMAN_RHO],
        ...
        for data_idx, data in enumerate(datasets):
            ...
            for rep in representations:
                ...
                for k, method in enumerate(
                    dim_results[metrics[0]][dimensions[0]][cv][data].keys()
                ):
                    ...
                    first_metric = [
                        dim_results[metrics[0]][d][cv][data][method][rep][None][
                            metrics[0]
                        ]
                        for d in dimensions
                    ]
                    ...
                    average_metric = (
                        np.average(first_metric, axis=1)
                        if correct
                        else np.nanmean(first_metric, axis=1)
                    )
                    one_minus_mean_nmses = np.ones(len(dimensions)) - average_metric
                    std_nmses = np.nanstd(
                        first_metric, axis=1
                    )  # TODO: divide by sqrt of N
                    second_metric = [
                        dim_results[metrics[1]][d][cv][datasets[0]][method][rep][None][
                            metrics[1]
                        ]
                        for d in dimensions
                    ]
                    ...
                    mean_rhos = (
                        np.average(second_metric, axis=1)
                        if correct
                        else np.nanmean(second_metric, axis=1)
                    )
                    std_rhos = np.nanstd(second_metric, axis=1)
                    # fit regression line
                    _ds = np.array(
                        _ds
                    )  # VAE arrays, repeat last dim-observations for length of array, but len(_ds) needs to be len(observations)
                    popt_mse, _pcov = curve_fit(
                        f, _ds, np.nan_to_num(one_minus_mean_nmses)[: len(_ds)]
                    )
                    popt_r, _pcov = curve_fit(
                        f, _ds, np.nan_to_num(mean_rhos)[: len(_ds)]
                    )
                    mse_fit = np.nan_to_num(f(xx, *popt_mse))
                    r_fit = np.nan_to_num(f(xx, *popt_r))
                    ax[i, j_idx].plot(
                        xx, mse_fit, linestyle="dotted", c=colors[k], alpha=0.5
                    )
                    ax[i, j_idx + 1].plot(
                        xx, r_fit, linestyle="dotted", c=colors[k], alpha=0.5
                    )
                    for r, mse, std_r, std_mse, _d in zip(
                        mean_rhos, one_minus_mean_nmses, std_rhos, std_nmses, _ds
                    ):
                        ax[i, j_idx].errorbar(
                            _d,
                            mse,
                            yerr=std_mse,
                            marker=shapes[data_idx],
                            c=colors[k],
                            ms=4,
                            mew=2,
                            linestyle="--",
                            label=f"{data}-{method}",
                            alpha=(0.5 + int(correct) * 0.5),
                        )  # shade if values are missing by alpha
                        ax[i, j_idx + 1].errorbar(
                            _d,
                            r,
                            yerr=std_r,
                            marker=shapes[data_idx],
                    ...
                    ax[i, j_idx].set_ylim([0.0, 1.1])
                    ax[i, j_idx + 1].set_ylim([0.0, 1.1])
                    ...
                    ax[i, j_idx + 1].tick_params(
                        axis="both", which="major", labelsize=9
                    )
                    ax[i, j_idx + 1].tick_params(
                        axis="both", which="minor", labelsize=5
                    )
                    ax[i, j_idx + 1].grid(True, color="k", alpha=0.25)
                    ax[i, j_idx + 1].set_title(f"{rep}", fontsize=9)
                    ax[i, j_idx + 1].set_ylabel(
                        f"{cv}\n 1-NMSE", rotation=90, fontsize=10
                    )
                    ax[i, j_idx + 1].set_ylabel(
                        f"{metrics[1]}", rotation=90, fontsize=10
                    )
                    ax[i, j_idx].set_xlabel("Dimensions")
                j_idx += 2
    ax[i, j_idx - 1].legend(legend_set, prop={"size": 6})
    plt.suptitle(
        f"Regression in lower dimensions {dim_reduction} \n on {str(datasets)}"
    )
    plt.tight_layout()
    plt.savefig(
        f"./results/figures/dim_reduction/lower_dim{dimensions}_{datasets}_{representations}_{metrics}_{dim_reduction}.pdf",
        bbox_inches="tight",
    )
    plt.savefig(
        f"./results/figures/dim_reduction/lower_dim{dimensions}_{datasets}_{representations}_{metrics}_{dim_reduction}.png",
        bbox_inches="tight",
    )
    plt.show()
