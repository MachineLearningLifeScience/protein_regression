...
def plot_lower_dim_results(datasets: List[str], algorithms: List[str], representations: List[str], cv_types: List[str],
    ...
    for i, cv in enumerate(cv_names):
        ...
        for rep in representations: 
            for k, method in enumerate(dim_results[metrics[0]][dimensions[0]][cv][datasets[0]].keys()):
                # Note: currently only plots one-element datasets list, otherwise too large/convuluted plot
                first_metric = [dim_results[metrics[0]][d][cv][datasets[0]][method][rep][None][metrics[0]] for d in dimensions]
    ...
    plt.tight_layout()
    plt.savefig(f"./results/figures/dim_reduction/lower_dim{dimensions}_{datasets[0]}_{representations}_{metrics}_{dim_reduction}", bbox_inches='tight')
    plt.show()
