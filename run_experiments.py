...
PROBLEM_CASES = ["UBQT", "BRCA"] #UBQT: transformer, VAE breaks, BRCA: Random: transformer, VAE breaks
# DONE: "MTH3", "TIMB", "CALM", "UBQT", "BRCA"

datasets = ["MTH3", "TIMB", "CALM", "1FQG", "UBQT", "BRCA"] # "MTH3", "TIMB", "CALM", "1FQG", "UBQT", "BRCA"
representations = [TRANSFORMER, VAE, ONE_HOT] # TRANSFORMER, VAE TODO:, ONE_HOT
augmentations = [NO_AUGMENT]
train_test_splitters = [RandomSplitter()] # [lambda dataset: BlockPostionSplitter(dataset)] # [BlockPostionSplitter, RandomSplitter]  #  

method_factories = [get_key_for_factory(f) for f in [GPLinearFactory]]#, GPSEFactory, GPLinearFactory] #, BayesRegressorFactory]
def run_experiments():
...
def run_augmentation_experiments():
    for dataset in ["UBQT", "CALM", "1FQG"]:#, "UBQT", "CALM"
        print(dataset)
