        ...
        for model in models:
            ...
            for rand in random_seeds:
                X_train, X_test, y_train, y_test = train_test_split(
                    X, y, test_size=0.3, random_state=rand
                )
                ...
                try:
                    ...
                    print(
                        f"Experiment: {model.get_name()} in d={d} not stable, prediction failed!"
                    )
                    _mu, unc = np.full(y_test.shape, np.nan), np.full(
                        y_test.shape, np.nan
                    )
                baseline = np.mean(
                    np.square(
                        y_test - np.repeat(np.mean(y_test), len(y_test)).reshape(-1, 1)
                    )
                )
...
if __name__ == "__main__":
    ...
    squareexp_gp = GPonRealSpace(
        kernel_factory=lambda: SquaredExponential(), optimize=True
    )
    ...
    for d, dim_entry in results.items():
        for m, entry in dim_entry.items():
            for _mse, _mll, _r, _unc in zip(
                entry["mse"], entry["mll"], entry["r"], entry["unc"]
            ):
                df = pd.concat(
                    [
    ...
    for d in dimensions:
        ...
        for method in results[d].keys():
            ...
            try:
                axs[3].hist(
                    np.concatenate(__df.unc.values),
                    10,
                    label=f"d={d};{method}",
                    alpha=0.5,
                )
