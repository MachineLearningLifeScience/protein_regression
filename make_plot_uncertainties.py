...
from util.mlflow.constants import NO_AUGMENT, ROSETTA, TRANSFORMER, VAE, VAE_RAND, VAE_AUX, EVE
...
if __name__ == "__main__":
        ...
        #                   augmentations = augmentations, number_quantiles=number_quantiles, optimize=True, 
        #                   d=d, dim_reduction=dim_reduction)
        ### MAKE UNCERTAIN PLOTS TASK SPLITTER
        plot_uncertainty_eval(datasets=["TOXI"], reps=reps,
                          algos=algos, train_test_splitter=WeightedTaskSplitter("TOXI", threshold=3), 
                          augmentations = augmentations, number_quantiles=number_quantiles, optimize=True, 
                          d=d, dim_reduction=dim_reduction)
