...
def align_meta_files(mlflow_results_dir: str):
    ...
    for exp_dir in os.listdir(mlflow_results_dir):
        ...
        for run_dir in os.listdir(mlflow_results_dir + os.sep + exp_dir):
            ...
            artifact_location_exists = os.path.isdir(
                meta_artifacts.replace("file://", "")
            )
            # check we're looking at a run directory for changes
            results_subdir_is_not_a_directory = not os.path.isdir(
                mlflow_results_dir + os.sep + exp_dir + os.sep + run_dir
            )
            ...
            try:
                ...
                continue
            if (
                not run_exp_id == meta_exp_id or not artifact_location_exists
            ):  # misaligned output files or wrong artifact location
                change_cnt += 1
                run_meta_file["experiment_id"] = meta_exp_id
                # adjust for mlflow results artifact directory
                run_meta_file["artifact_uri"] = (
                    mlflow_results_dir
                    + os.sep
                    + meta_exp_id
                    + os.sep
            ...
            assert os.path.isdir(
                run_meta_file.get("artifact_uri").replace("file://", "")
            )
...
if __name__ == "__main__":
    ...
    MLFLOW_DIRECTORY = (
        Path(__file__).parent.parent.parent.resolve() / "results" / "mlruns"
    )
