...
def test_return_indices_lists():
    ...
    """
    train, _, test = positional_splitter(ssl_X, ref_X, val=False, offset=1, pos_per_fold=1)
...
def test_pos_per_fold():
    ...
    for pos in range(1, int(L/10)):
        print(pos)
        train, _, test = positional_splitter(ssl_X, ref_X, val=False, offset=0, pos_per_fold=pos)
        assert len(train) == len(test) == np.ceil(len(unique_mutations)/pos)
        for idx, _test in enumerate(test):
            pos_idx = idx*pos
            mutated_positions_at_idx = np.hstack([np.where(mutation_pos==p)[0] for p in list(unique_mutations[pos_idx:pos_idx+pos])])
...
def test_no_offset_no_sequences_excluded_():
    ...
    """
    train, _, test = positional_splitter(ssl_X, ref_X, val=False, offset=0, pos_per_fold=1)
...
def test_with_offset_sequences_are_excluded():
    ...
    for offset in range(10):
        train, _, test = positional_splitter(ssl_X, ref_X, val=False, offset=offset, pos_per_fold=1)
...
def test_with_offset_no_overlaps():
    ...
    for offset in range(1, 10):
        ...
        train, _, test = positional_splitter(seqs=ssl_X, query_seq=ref_X, val=False, offset=offset, pos_per_fold=pos)
        for idx, (_train, _test) in enumerate(zip(train, test)):
            ...
            for k in range(1, offset):
                ...
                assert all(np.isin(mutated_positions_at_exclusion_idx, _train, invert=True))
...
def test_validation_flag_false():
    ...
    """
    train, val, test = positional_splitter(ssl_X, ref_X, val=False, offset=1, pos_per_fold=20)
    assert len(train) == len(val) == len(test)
    assert all([[not v for v in _val] for _val in val])


def test_validation_flag_return():
    train, val, test = positional_splitter(ssl_X, ref_X, val=True, offset=1, pos_per_fold=10)
...
def test_sequences_equal_raises_error():
    ...
    with pytest.raises(ValueError):
        positional_splitter(seqs=stacked_reference, query_seq=ref_X, val=True, offset=1, pos_per_fold=1)
