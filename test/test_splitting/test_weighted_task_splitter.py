...
def test_weight_reference_against_matrix_computation():
    ...
    ws = np.array(ws)[:, 0].reshape(M, N) # initial euclidean distances account for first column of distance matrix!
...
def average_log_weight_REFERENCE(alphas, X_s, Y_s, X_p, Y_p) -> float:
    ...
    ws = []
    for i in range(M):
        _w = weight_REFERENCE(alphas=alphas, x_p=X_p[i], y_p=Y_p[i], X_s=X_s, Y_s=Y_s, eta=eta)
        ws.append(_w)
