...
test_methods = [
    UncertainRandomForest().get_name(),  # TODO
...
result_set_opt = list(
    product(test_datasets[:2], test_representations[:-1], test_methods[:-1], seeds)
)
result_set_toxi = list(
    product(
        ["TOXI"],
        [ESM],
        [GPonRealSpace(kernel_factory=lambda: Matern52()).get_name()],
...
@pytest.mark.parametrize(
    "dataset,representation,method,protocol_factory,metric,dim",
    list(result_set_one + result_set_two + result_set_tasks + result_set_toxi),
)
def test_main_results_exist(
    dataset, representation, method, protocol_factory, metric, dim
):
    """
    ...
    for protocol in protocols:
        ...
        if ("pos" in protocol.get_name().lower() and dataset == "TOXI") or (
            "fract" in protocol.get_name().lower() and dataset == "TOXI"
        ):
            continue
        if exists(cached_filename):
            ...
            try:
                results = load_results_dict_from_mlflow(
                    datasets=[dataset],
                    algos=[method],
                    metrics=[metric],
                    reps=[representation],
                    train_test_splitter=[protocol],
                    cache=True,
        ...
        protocol_name = (
            "Fractional"
            if "fract" in protocol.get_name().lower()
            else protocol.get_name()
        )
        ...
        assert (
            len(np.array(metric_result).flatten()) > 1
        )  # more than one result per split
...
def test_optimization_results_exist(dataset, representation, method, seed):
    ...
    assert (
        len(
            np.array(
                results[seed][dataset][method][representation][None][OBSERVED_Y]
            ).flatten()
        )
        > 1
    )
...
test_set_ablation_mutation = list(
    product(
        ["TOXI"],
        [ONE_HOT, EVE, ESM],
        [GPonRealSpace(kernel_factory=lambda: Matern52()).get_name()],
...
test_set_ablation_embeddings_regressors = list(
    product(
        ["1FQG", "UBQT", "TIMB", "MTH3", "BRCA"],
...
ablation_test_iterator = (
    test_set_ablation_mutation
    + test_set_ablation_dims
    + test_set_ablation_embeddings_regressors
)


@pytest.mark.parametrize(
    "dataset,representation,method,protocol,metric,dim,dim_reduction",
    list(ablation_test_iterator),
)
def test_ablation_results_exist(
    ...
    assert (
        len(results_dict[protocol][dataset][method][representation][None][metric]) > 1
    )
