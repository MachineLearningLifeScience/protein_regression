...
@pytest.mark.parametrize(
    "dataset,representation,method,protocol,metric,dim", list(test_iterator)
)
def test_rho_for_constant_predictions(
    dataset: List[str],
    ...
    if exists(cached_filename):
        ...
        try:
            ...
            print(
                f"No entries found for {dataset,representation,method,protocol,metric,dim}..."
            )
            return missing_pass
    ...
    metric_results = np.array(
        [
            results_dict[dataset][method][representation][None][s][metric]
            for s in results_dict[dataset][method][representation][None].keys()
        ]
    )
    predictions = np.array(
        [
            results_dict[dataset][method][representation][None][s]["pred"]
            for s in results_dict[dataset][method][representation][None].keys()
        ]
    )
    ...
    for idx in np.where(nan_predictions)[0]:
        ...
        np.testing.assert_allclose(
            predictions[idx],
            np.repeat(predictions[idx][0], len(predictions[idx])),
            rtol=0.0039,
        )
