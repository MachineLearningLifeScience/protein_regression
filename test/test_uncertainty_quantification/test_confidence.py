...
class TestConfidence:
    ...
    def test_mismatch_results_uncertainties(self):
        with pytest.raises(AssertionError):
            ranking_confidence_curve(
                _test_losses_results[:-2],
                uncertainties=_uncalibrated_results_uncertainties,
            )

    def test_auco_naive(self):
        quantile = range(10)
        uncalibrated_unc, uncalibrated_oracle = ranking_confidence_curve(
    ...
    def test_auco_zero_if_perfect(self):
        unc, oracle = ranking_confidence_curve(
            _test_losses_results, _calibrated_results_uncertainties
        )
        perfect_auco = area_confidence_oracle_error(unc, oracle)
        assert perfect_auco == 0.0

    def test_auco_smaller_if_calibrated(self):
        cal_unc, cal_oracle = ranking_confidence_curve(
            _test_losses_results, _calibrated_results_uncertainties
        )
        uncal_unc, uncal_oracle = ranking_confidence_curve(
            _test_losses_results, _uncalibrated_results_uncertainties
        )
    ...
    def test_naive_confidence_curve(self):
        ...
        # compare with
        _losses_unc, _losses_oracle = ranking_confidence_curve(
            example_losses, example_uncertainties
        )
    ...
    def test_uncertain_outlier(self):
        ...
        """
        loss_ranked_by_confidence, _ = ranking_confidence_curve(
            _test_losses_results, _uncalibrated_results_uncertainties
        )
    ...
    def test_loss_is_oracle(self):
        ...
        """
        loss_ranked_by_confidence, loss_ranked_by_loss = ranking_confidence_curve(
            _test_losses_results, _calibrated_results_uncertainties
        )
    ...
    def test_imperfect_loss_higher_than_oracle(self):
        ...
        loss_by_conf, loss_oracle = ranking_confidence_curve(
            unordered_test_loss, _calibrated_results_uncertainties
        )
    ...
    def test_random_sampling_constant(self):
        ...
        ranked_confidence, _ = ranking_confidence_curve(
            _test_losses_results, random_uncertainties
        )
        np.testing.assert_allclose(
            ranked_confidence[:-1], np.ones(len(ranked_confidence) - 1), rtol=0.1
        )

    def test_decreasing_ratio_calibrated(self):
        ranked_conf, _ = ranking_confidence_curve(
            _test_losses_results, _calibrated_results_uncertainties
        )
    ...
    def test_decreasing_ratio_uncalibrated(self):
        ...
        ratio = decreasing_ratio(ranked_conf)
        np.testing.assert_almost_equal(ratio, 0.0)
