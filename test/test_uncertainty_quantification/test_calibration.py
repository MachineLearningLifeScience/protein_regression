...
_uncalibrated_results_uncertainties = np.sort(
    np.random.normal(loc=0.3, scale=0.5, size=n)
)[::-1]
_calibrated_results_uncertainties = np.sort(
    np.random.normal(loc=0.2, scale=0.01, size=n)
)
...
class TestCalibration:
    def test_naive_calibration(self):
        ...
        _frac_quantiles, _unc_quantiles = confidence_based_calibration(
            _test_mean_predictions,
            _calibrated_results_uncertainties,
            y_ref_mean=pred_mean,
        )
    ...
    def test_perfect_calibration(self):
        ...
        frac_quantiles, _unc_quantiles = confidence_based_calibration(
    ...
    def test_random_calibration_flat(self):
        ...
        frac_quantiles, unc_quantiles = confidence_based_calibration(
            _test_mean_predictions, random_uncertainties
        )
        np.testing.assert_array_less(
            frac_quantiles[: int(len(frac_quantiles) / 2)], 0.25
        )
    ...
    def test_ece_calibrated_lower_random(self):
        calibrated_frac_quantiles, calibrated_unc_quantiles = (
            confidence_based_calibration(
                _test_mean_predictions, _calibrated_results_uncertainties
            )
        )
        ...
        rand_frac_quantiles, rand_unc_quantiles = confidence_based_calibration(
            _test_mean_predictions, np.random.normal(0.3, 0.1, n)
        )
    ...
    def test_ece_uncalibrated_greater_random(self):
        uncal_frac_quantiles, uncal_unc_quantiles = confidence_based_calibration(
            _test_mean_predictions, _uncalibrated_results_uncertainties
        )
        uncal_ece = expected_calibration_error(
            uncal_frac_quantiles, uncal_unc_quantiles
        )
        rand_frac_quantiles, rand_unc_quantiles = confidence_based_calibration(
            _test_mean_predictions, np.random.normal(0.3, 0.1, n)
        )
    ...
    def test_ence_calibrated_lesser_random(self):
        ence = expected_normalized_calibration_error(
            _test_losses_results, _calibrated_results_uncertainties
        )
        random_unc = np.random.normal(0.2, 0.5, n)
        random_ence = expected_normalized_calibration_error(
            _test_losses_results, random_unc
        )
        assert ence <= random_ence

    def test_ence_uncalibrated_great_random(self):
        ence = expected_normalized_calibration_error(
            _test_losses_results, _uncalibrated_results_uncertainties
        )
        random_unc = np.random.normal(0.2, 0.5, n)
        random_ence = expected_normalized_calibration_error(
            _test_losses_results, random_unc
