...
class TestModuleIntegration:
    def test_reliability_prep(self):
        ...
        count, perc, ECE, Sharpness = prep_reliability_diagram(true=true_values, preds=_test_mean_predictions, uncertainties=_calibrated_results_uncertainties, number_quantiles=11)
        fractions, uncertainties = confidence_based_calibration(_test_mean_predictions, _calibrated_results_uncertainties)
    ...
    def test_confidence_curve(self):
        q_err, o_err = ranking_confidence_curve(losses=_test_losses_results, uncertainties=_calibrated_results_uncertainties)
        _q_err, _o_err = quantile_and_oracle_errors(uncertainties=_calibrated_results_uncertainties, errors=_test_losses_results, number_quantiles=10)
