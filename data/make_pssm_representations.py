...
BACKGROUND_AA_FREQUENCIES = {
    'A': 0.0903, 'C': 0.0129, 'D': 0.0547, 'E': 0.0624, 'F': 0.0388,
    'G': 0.0727, 'H': 0.0222, 'I': 0.0553, 'K': 0.0493, 'L': 0.0985,
    'M': 0.0233, 'N': 0.0379, 'P': 0.0499, 'Q': 0.0380, 'R': 0.0584,
    'S': 0.0682, 'T': 0.0555, 'V': 0.0686, 'W': 0.0130, 'Y': 0.0288,
}
...
def calculate_pssm(hmm_emissions, background_frequencies):
    ...
    for position_emissions in hmm_emissions:
        pssm_row = []
        for aa, emission in zip(background_frequencies.keys(), position_emissions): # NOTE: background frequencies have to be ordered
...
def compute_pssm_and_persist(data_key: str, output_filepath: Path, hmm_filename_lookup=DATA_HMM_KVP, background_freq=BACKGROUND_AA_FREQUENCIES):
    ...
    assert hmm_aa_positions == list(background_freq.keys()), "AA mapping different between HMM and frequency dict!"
