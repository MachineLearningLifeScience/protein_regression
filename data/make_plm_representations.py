...
AVAILABLE_MODELS = {
    "esm1b": {"model_id": "esm1b_t33_650M_UR50S", "layer": 33},
    "esm1v": {"model_id": "esm1v_t33_650M_UR90S_1", "layer": 33},
    "esm2": {"model_id": "esm2_t36_3B_UR50D", "layer": 36},
        ...
        for i in range(1, len(sequences[s])):  # NOTE: skipping cls start token
            ...
            with torch.no_grad():
                token_probs = torch.log_softmax(
                    model(_batch_tokens_masked.to(device))["logits"], dim=-1
                )
...
def compute_esm_plls(
    model, seqs, seq_enc_alphabet, alphabet, dev, stepsize: int = 10
) -> np.ndarray:
    print("Compute PLL ...")
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(converted_sequences), stepsize)):
            ...
            log_probs = compute_pll(
                batched_sequences, model, batch_tokens, alphabet, device=dev
            )
...
def compute_esm_representation(
    model,
    seqs: List[int],
    alphabet,
    seq_enc_alphabet: dict,
    dev,
    stepsize: int = 10,
    rep_lyr: int = 33,
) -> np.ndarray:
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(converted_sequences), stepsize)):
            ...
            _, _, batch_tokens = batch_converter(data)
            results = model(
                batch_tokens.to(dev), repr_layers=[rep_lyr], return_contacts=False
            )
            representation = (
                results["representations"][rep_lyr].cpu().detach().numpy().mean(axis=1)
            )
...
def compute_prot_representation(
    model,
    seqs: List[str],
    alphabet,
    seq_enc_alphabet: dict,
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(converted_sequences), stepsize)):
            ...
            representation = (
                model_attn.last_hidden_state.cpu().detach().numpy().mean(axis=1)
            )
            rep.append(representation)
    print(f"Computed representations in shape (B, D) => {representation.shape}")
    rep_array = np.vstack(np.array(rep))
...
def load_model_from_repository(model_key, all_models=AVAILABLE_MODELS) -> tuple:
    if "esm" in model_key:
        model, alphabet = torch.hub.load(
            "facebookresearch/esm:main", all_models.get(model_key).get("model_id")
        )
    elif "prot" in model_key:
        model = T5EncoderModel.from_pretrained(
            f"Rostlab/{all_models.get(model_key).get('model_id')}"
        )
        alphabet = T5Tokenizer.from_pretrained(
            f"Rostlab/{all_models.get(model_key).get('model_id')}"
        )
    else:
...
def main(model_kvp=AVAILABLE_MODELS):
    ...
    parser.add_argument(
        "-d", "--data", type=str, choices=ALL_DATASETS, help="Dataset identifier"
    )
    parser.add_argument(
        "-m",
        "--model",
        ...
        help="pLM model identifier",
    )
    parser.add_argument(
        "-s", "--stepsize", type=int, default=25, help="Compute in steps of '-s'"
    )
    parser.add_argument(
        "--pll", action="store_true", help="Compute Pseudo-Log-Likelihoods"
    )
