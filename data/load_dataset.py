...
def __compute_observation_and_deduplication_indices(
    df,
    ...
    observations_start: int = np.argwhere(idx.values == True)[0][
        0
    ...
    if "mutant" in df.columns:
        ...
        duplicated_index = df[
            ["seqs_aa", "assay"]
        ].duplicated()  # find duplicates as sequence+assay repeats
    observation_idx = np.logical_and(
        idx, ~duplicated_index
    )  # combine observation AND INVERT duplicate for later selection
    anchored_duplicated_index = duplicated_index.set_axis(
        duplicated_index.index - observations_start
    )  # adjust index by first observation
    representation_index = np.invert(
        anchored_duplicated_index[anchored_duplicated_index.index >= 0]
    )
...
def load_plm(name: str, model_key="esm") -> Tuple[np.ndarray, np.ndarray]:
    if name == "1FQG":
        ...
        observation_idx, representation_idx = (
            __compute_observation_and_deduplication_indices(d)
        )
        d = d.loc[observation_idx]
        Y = np.vstack(d["assay"])
        X = pickle.load(open(join(base_path, f"blat_{model_key}_rep.pkl"), "rb"))[
            representation_idx
        ]
    elif name == "BRCA":
        ...
        if rep.shape[0] != Y.shape[0]:
            X = rep[
                representation_idx
            ]  # NOTE base esm is of shape representation_idx (not deduplicated and requires indexing)
        else:
            X = rep
    elif name == "CALM":
        ...
        d = d.loc[observation_idx]
        Y = np.vstack(d["assay"])
        X = pickle.load(open(join(base_path, f"calm_{model_key}_rep.pkl"), "rb"))[
            representation_idx
        ]
    elif name == "MTH3":
        ...
        d = d.loc[observation_idx]
        Y = np.vstack(d["assay"])
        X = pickle.load(open(join(base_path, f"mth3_{model_key}_rep.pkl"), "rb"))[
            representation_idx
        ]
    elif name == "TIMB":
        ...
        d = d.loc[observation_idx]
        Y = np.vstack(d["assay"])
        X = pickle.load(open(join(base_path, f"timb_{model_key}_rep.pkl"), "rb"))[
            representation_idx
        ]
    elif name == "UBQT":
        ...
        d = d.loc[observation_idx]
        Y = np.vstack(d["assay"])
        X = pickle.load(open(join(base_path, f"toxi_{model_key}_rep.pkl"), "rb"))[
            representation_idx[observation_idx == True]
        ]
    else:
        raise ValueError("Unknown dataset: %s" % name)
...
def load_eve(name):
    ...
    if len(latent_z) != len(Y):
        warnings.warn(
            f"no. EVE mutants {len(latent_z)} != no. observations {len(Y)}! \n Diff: {len(Y)-len(latent_z)} ..."
        )
...
def __load_eve_df(name) -> Tuple[np.ndarray, np.ndarray]:
    ...
    eve_df = pd.read_csv(
        join(base_path, f"EVE_{name.upper()}_2000_samples.csv")
    ).drop_duplicates()
    merged_df_eve_observations = pd.merge(
        observed_d, eve_df, on="mutations", validate="one_to_one"
    )
    wt = merged_df_eve_observations[merged_df_eve_observations.mutations == "wt"]
    merged_df_eve_observations = (
        merged_df_eve_observations.drop(index=wt.index)
        if wt is not None
        else merged_df_eve_observations
    )
    # add mutation positions and AA to DF
    if merged_df_eve_observations.mutations.str.contains(":").any():
        merged_df_eve_observations["last_mutation_position"] = (
            merged_df_eve_observations.mutations.apply(
                lambda x: x.split(":")[-1][1:-1]
            ).astype(int)
            + offset
        )
        merged_df_eve_observations["mut_aa"] = (
            merged_df_eve_observations.mutations.apply(lambda x: x.split(":")[-1][-1])
        )
    else:
        merged_df_eve_observations["last_mutation_position"] = (
            merged_df_eve_observations.mutations.str[1:-1].astype(int) + offset
        )
        merged_df_eve_observations["mut_aa"] = merged_df_eve_observations.mutations.str[
            -1
        ]
...
def __convert_log_sm_to_prob_vec(x: np.array):
    ...
    _x = x.copy()
    # reshape to [N, L, |AA|]
    x_vec = np.concatenate(x, axis=-1).reshape(
        (
            _x.shape[0],
            np.concatenate(_x, axis=-1).shape[0],
            np.concatenate(_x, axis=0).shape[1],
        )
    )
    probits = np.exp(x_vec)
    # convert log-softmax output to probability vectors
    X = probits / np.sum(probits, axis=-1)[..., None]
    assert np.all(np.sum(X, axis=-1)) == 1.0, "Probability Vector does not sum to one!"
    return X
...
def load_dataset(
    name: str, desired_alphabet=None, representation=ONE_HOT, augmentation=None
) -> tuple:
    print(f"Loading {name} representation: {representation}...")
    if desired_alphabet is not None and representation is not ONE_HOT:
        raise ValueError(
            "Desired alphabet can only have a value when representation is one hot!"
        )
    ...
    if representation == ONE_HOT:
        ...
        #     X, Y, A = load_vae(name, vae_suffix="AUX_VAE_reps_RANDOM_VAL")
            ...
            missed_assay_indices = (
                None  # EVE protocol covers all possible single variants
            )
        elif (
            representation == EVE_DENSITY
        ):  # TODO: add VAE_Density representation again
        ...
        X = X.astype(
            np.float64
        )  # in the one-hot case we want int64, that's why the cast is in this position
    Y = Y.astype(np.float64)
    # Augmentation Loading:
    if augmentation == ROSETTA or (
        augmentation == EVE_DENSITY and representation is not EVE
    ):
        (
            S,
            A,
        ) = load_augmentation(name, augmentation)
        data_S = (
            load_one_hot(name, desired_alphabet=desired_alphabet)[0]
            if representation != EVE
            else eve_S
        )  # EVE sequences are distinct from base data sequences
        X, Y, A, missed_assay_indices = join_X_A_by_available_sequences(
            X, Y, data_S=data_S, S=S, A=A, name=name
        )
    assert Y.shape[1] == 1
    assert X.shape[0] == Y.shape[0]
...
def load_augmentation(name: str, augmentation: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if augmentation == ROSETTA:
        ...
        raise NotImplementedError(
            f"Requested augmentation {augmentation} does not exist!"
        )
    return S, A


def join_X_A_by_available_sequences(
    X: np.ndarray,
    Y: np.ndarray,
    data_S: np.ndarray,
    S: np.ndarray,
    ...
    missed_assay_indices = None
    A = (
        A if A.shape[-1] == 1 else A[:, np.newaxis]
    )  # add augmentation vector from Rosetta | EVE | VAE to data-matrix
    ...
    idx_join = np.in1d(
        [str(_os) for _os in _lh_sequences], [str(_s) for _s in _rh_sequences]
    )  # str representation required for "in" assessment
    ...
    if np.isnan(X).any():
        warnings.warn(
            f"NaN values encountered in {name} augmentation! Shape: {X.shape}"
        )
...
def get_wildtype_and_offset(name: str) -> Tuple[str, int]:
    ...
    if name == "1FQG" or name == "BLAT":  # beta-lactamase
        ...
        sequence_offset = __parse_sequence_offset_from_alignment(
            "alignments/BLAT_ECOLX_hmmerbit_plmc_n5_m30_f50_t0.2_r24-286_id100_b105.a2m"
        )
        wt = d["seqs"][0].astype(np.int64)
    elif name == "BRCA" or name == "BRCA_BRCT":
        d = pickle.load(open(join(base_path, "brca_data_df.pkl"), "rb"))
        sequence_offset = __parse_sequence_offset_from_alignment(
            "alignments/BRCA1_HUMAN_BRCT_1_b0.3.a2m"
        )
        wt = d["seqs"][0].astype(np.int64)
    elif name == "CALM":
        d = pickle.load(open(join(base_path, "calm_data_df.pkl"), "rb"))
        sequence_offset = __parse_sequence_offset_from_alignment(
            "alignments/CALM1_HUMAN_1_b0.5.a2m"
        )
        wt = d["seqs"][0].astype(np.int64)
    elif name == "MTH3":
        d = pickle.load(open(join(base_path, "mth3_data_df.pkl"), "rb"))
        sequence_offset = __parse_sequence_offset_from_alignment(
            "alignments/MTH3_HAEAESTABILIZED_1_b0.5.a2m"
        )
        wt = d["seqs"][0].astype(np.int64)
    elif name == "TIMB":
        d = pickle.load(open(join(base_path, "timb_data_df.pkl"), "rb"))
        sequence_offset = __parse_sequence_offset_from_alignment(
            "alignments/TRPC_THEMA_1_b0.5.a2m"
        )
        wt = d["seqs"][0].astype(np.int64)
    elif name == "UBQT":
        d = pickle.load(open(join(base_path, "ubqt_data_df.pkl"), "rb"))
        sequence_offset = __parse_sequence_offset_from_alignment(
            "alignments/RL401_YEAST_1_b0.5.a2m"
        )
        wt = d["seqs"][0].astype(np.int64)
    elif name == "TOXI":
        d = pickle.load(open(join(base_path, "toxi_data_df.pkl"), "rb"))
        sequence_offset = __parse_sequence_offset_from_alignment(
            "alignments/parEparD_3.a2m"
        )
...
def compute_mutations_csv_from_observations(
    observations_file: str, offset: int = 0
) -> None:
    """
...
def __load_assay_df(name: str):
    ...
    if "mutant" in df.columns:
        ...
        if df.mutant.str.contains(":").any():
            df["last_mutation_position"] = (
                df.mutant.apply(lambda x: x.split(":")[-1][1:-1]).astype(int) + offset
            )
            df["mut_aa"] = df.mutant.apply(lambda x: x.split(":")[-1][-1])
        else:
            df["last_mutation_position"] = (
                df.mutant.str.slice(1, -1).astype(int) + offset
            )
        ...
        df["mutation_idx"] = [
            [idx + 1 + offset for idx in np.argwhere(df.seqs[obs] != wt_sequence)[0]]
            for obs in range(len(df))
        ]
        df["last_mutation_position"] = [
            int(mut[-1]) if mut else 0 for mut in df.mutation_idx
        ]
        # we use the LAST mutation value (-1) from the index,in case of multi-mutation values in there
        df["mut_aa"] = [
            alphabet.get(int(seq[idx - 1 - offset]))
            for seq, idx in zip(df.seqs, df.last_mutation_position)
        ]
    return df
...
def __load_rosetta_augmentation_df(name: str) -> pd.DataFrame:
    ...
    rosetta_df = pd.read_csv(
        join(base_path, "{}_single_mutation_rosetta.csv".format(name.lower()))
    )
    wt_sequence, offset = get_wildtype_and_offset(name)
    if offset != 0.0 and rosetta_df.position.astype(int)[0] == 1:
    ...
    observations_df = observations_df[
        ["seqs", "assay", "last_mutation_position", "mut_aa"]
    ].drop_duplicates(subset=["assay", "last_mutation_position", "mut_aa"])
    joined_df = pd.merge(
        rosetta_df,
        observations_df,
        how="right",
        left_on=["position", "mut_aa"],
