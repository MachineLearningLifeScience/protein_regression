...
if __name__ == "__main__":
    for data_key in ALL_DATASETS:
        ...
        output_path = Path(__file__).parent.resolve() / "files" / "fastas" / f"{data_key}_wt.fasta"
        converted_wt_sequence = "".join([list(seq_enc_alphabet.keys())[list(seq_enc_alphabet.values()).index(s_i)] for s_i in wt]) 
        # persist results
