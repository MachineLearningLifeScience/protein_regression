...
class BioSplitter(AbstractTrainTestSplitter):
    ...
    def __init__(
        self, dataset, n_mutations_train: int = 3, n_mutations_test: int = 4, n_splits=5
    ):
        super().__init__()
        self.dataset = dataset
    ...
    def split(self, X, representation=ONE_HOT, missed_indices=None):
        ...
        """
        _X = load_sequences_of_representation(
            self.dataset, representation=representation
        )
        ...
        if self.n_mutations_train == self.n_mutations_test:
            ...
            np.random.shuffle(
                n_mutants_indices
            )  # random permutation of target mutations
            for idx in range(self.n_splits):
                test_indices.append(
                    n_mutants_indices[N_test * idx : N_test * idx + N_test]
                )
            ...
            train_indices = np.where(diff_to_wt <= self.n_mutations_train)[0][
                np.newaxis, :
            ]
            test_indices = np.where(diff_to_wt == self.n_mutations_test)[0][
                np.newaxis, :
            ]
        return train_indices, None, test_indices
    ...
    def get_name(self):
        splitter_name = (
            self.name + str(self.n_mutations_train) + "_" + str(self.n_mutations_test)
        )
        return splitter_name
...
class WeightedTaskSplitter(AbstractTrainTestSplitter):
    def __init__(
        ...
        self.X_s_N_limit = (
            limit_N_Xs  # hard limit, due to optimization scaling inefficiencies
        )
        self.eta = 0.75
    ...
    def split(self, X, y):
        ...
        for _ in range(self.n_splits):
            ...
            s_idx = s_idx[
                : self.X_s_N_limit
            ]  # limit amount of X_s for later optimization
            ...
            p_vec = (
                np.concatenate([np.ones(len(p_idx)), avrg_weights]) / training_N
            )  # normalize to make it p-measure
            assert idx_vec.shape == p_vec.shape
            train_idx = np.random.choice(
                idx_vec, size=int(np.ceil(training_N)), p=p_vec
            )
            train_idx_list.append(train_idx)
            test_idx_list.append(test_idx)
        return train_idx_list, None, test_idx_list

    def split_Xy_by_mutations(
        self, X: np.ndarray
    ...
    def split_Xy_by_observations(self, y) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        p_idx = np.array(
            [
                ...
                if _y >= self.threshold and bool(np.random.binomial(1, self.p_accept))
            ]
        )  # functional variants
    ...
    def constrained_weight_sum(self, alphas):
        ...
        N = self.X_s.shape[0]
        w = self.weight(
            alphas, self.X_s, self.Y_s, self.X_s, self.Y_s, self.eta
        )  # returns NxN matrix from euclidean distances
    ...
    def _optimize(self, X, y):
        ...
        res = minimize(
            self.average_log_weight,
            alphas0,
            method="SLSQP",
            args=(X, y),
            constraints=cons,
            options={"disp": True},
        )  # jac=self.average_log_w_der,
        optimal_alphas = res.x
        return optimal_alphas

    def average_log_weight(
        self, alphas: np.ndarray, X_p: np.ndarray, Y_p: np.ndarray
    ) -> float:
        """
        ...
        """
        assert (
            alphas.shape[0] == self.X_s.shape[0] == self.Y_s.shape[0]
        ), "Alpha required with same dimensions as X_s and Y_s!"
        M = X_p.shape[0]
        _w = self.weight(
            alphas=alphas, x_p=X_p, y_p=Y_p, X_s=self.X_s, Y_s=self.Y_s, eta=self.eta
        )
    ...
    def weight(
        ...
        x_p_y_p = (
            x_p_y_p if x_p_y_p.shape[0] != x_p_y_p.shape[-1] else x_p_y_p[np.newaxis, :]
        )
...
class PositionSplitter(AbstractTrainTestSplitter):
    ...
    def __init__(
        self, dataset: str, positions: int = 15, missing_indices: np.ndarray = None
    ):
        super().__init__()
        self.wt, _ = get_wildtype_and_offset(dataset)
        self.dataset = dataset
    ...
    def split(self, X, setting: Tuple, missed_indices: np.ndarray = None):
        ...
        _X = load_sequences_of_representation(
            self.dataset, representation=representation, augmentation=augmentation
        )  # position splitting by original sequence positions - not encoding
        if missed_indices is not None and len(_X) != len(X):
            _X = np.delete(_X, missed_indices, axis=0)
        assert _X.shape[0] == X.shape[0]
        return positional_splitter(
            _X, self.wt, val=False, offset=4, pos_per_fold=self.positions
        )
...
class BlockPostionSplitter(AbstractTrainTestSplitter):
    ...
    def split(self, X):  # TODO broken for TOXI
        return positional_splitter(
            X, self.wt, val=False, offset=4, pos_per_fold=self.pos_per_fold
        )


def positional_splitter(
    seqs, query_seq, val: bool, offset: int, pos_per_fold: int
    ...
    for i in range(len(np.unique(mut_pos)) // (pos_per_fold) + 1):
        ...
        train_mut = list(unique_mut[: max(counter - offset, 0)]) + list(
            unique_mut[counter + pos_per_fold + offset :]
        )

        if offset > 0:
            buffer_mut = list(unique_mut[max(counter - offset, 0) : counter]) + list(
                unique_mut[counter + pos_per_fold : counter + pos_per_fold + offset]
            )
        else:
            buffer_mut = []
        ...
        if val:
            val_mut = [
                ...
                np.hstack(
                    [
                        unique_mut[: int(1 / 6 * pos_per_fold)],
                        unique_mut[-int(1 / 6 * pos_per_fold) :],
                    ]
                ),
                unique_mut[: int(1 / 3 * pos_per_fold)],
        ...
        verify_num_mut = (
            len(test_mut) + len(train_mut) + len(buffer_mut) + len(val_mut[i])
        )
        verify_num_idx = (len(test_idx) + len(train_idx) + len(buffer_idx)) + len(
            val_idx
        )
        assert (
            len(list(set(test_mut).intersection(set(train_mut)))) == 0
        ), "test and train idx overlap"
        assert (
            len(list(set(train_idx).intersection(set(test_idx)))) == 0
        ), "test and train idx overlap"
        assert len(unique_mut) == verify_num_mut, (
            "Something wrong with number of positions/mutations. Number of idx: "
            + str(verify_num_idx)
            + "Number of mut:"
            + str(verify_num_mut)
        )
