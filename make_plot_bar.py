...
from util import compute_delta_between_results, parse_baseline_mutation_observations
...
from util.mlflow.convenience_functions import (
    get_mlflow_results,
...
from visualization.plot_metric_for_dataset import (
    barplot_metric_augmentation_comparison,
    barplot_metric_comparison,
    barplot_metric_comparison_bar,
    barplot_metric_comparison_bar_splitting,
    barplot_metric_mutation_comparison,
    barplot_metric_mutation_matrix,
    errorplot_cv_comparison,
    errorplot_metric_comparison,
...
def plot_metric_comparison(
    ...
    results_dict = get_mlflow_results(
        datasets=datasets,
        algos=algos,
        reps=reps,
        metrics=metrics,
    ...
    for metric in metrics:
        if metric == MSE:
            barplot_metric_comparison(
                metric_values=results_dict, cvtype=cvtype, metric=metric
            )
        if metric == SPEARMAN_RHO:
            errorplot_metric_comparison(
                metric_values=results_dict,
                cvtype=cvtype,
                metric=metric,
                plot_reference=True,
            )
...
def plot_metric_comparison_bar(
    ...
    if cached_results and exists(cached_filename):
        ...
        results_dict = load_results_dict_from_mlflow(
            datasets,
            algos,
            metrics,
            reps,
            ...
            cache_fname=cached_filename,
            optimized=optimized,
            dim=dim,
            dim_reduction=dim_reduction,
        )
    cvtype = (
        str(set([splitter.get_name()[:12] for splitter in train_test_splitter]))
        + f"d=full"
    )
    for metric in metrics:
        try:
            ...
            print(
                f"Error figure: d={datasets}, a={algos}, r={reps}, cv={cvtype}, metric={metric}"
            )


def plot_cv_comparison_error(
    datasets: List[str],
    algos: List[str],
    metrics: List[str],
    reps: List[str],
    ...
    for metric in metrics:
        errorplot_cv_comparison(
            metric_values=results_dict, metric=metric, savefig=savefig
        )


def plot_metric_comparison_bar_param_delta(
    datasets: List[str],
    algos: List[str],
    metrics: List[str],
    reps: List[str],
    ...
    for metric in [
        SPEARMAN_RHO,
        "R2",
    ]:  # NOTE: we have already computed the delta, s.t. MSE is already 1-NMSE difference, hence MSE no longer required
        barplot_metric_comparison_bar(
            metric_values=results_dict,
            cvtype=cvtype,
            metric=metric,
...
def plot_metric_comparison_bar_splitting(
    datasets: List[str],
    algos: List[str],
    metrics: List[str],
    reps: List[str],
...
def load_mutation_results_with_baseline(
    datasets: List[str],
    algos: List[str],
    metrics: str,
    reps: List[str],
    ...
    if cached_results and exists(cached_filename):
        ...
        for splitter in train_test_splitter:
            _dict = get_mlflow_results_artifacts(
                datasets=datasets,
                algos=algos,
                reps=reps,
                metrics=metrics,
            ...
            added_train_observations, test_observations, train_trues = (
                parse_baseline_mutation_observations(_dict)
            )
            for method in _dict.get(datasets[0]).keys():
                _dict[datasets[0]][method]["additive"] = {None: {}}
                for split, (train_obs, test_obs, train_data) in enumerate(
                    zip(added_train_observations, test_observations, train_trues)
                ):
                    _dict[datasets[0]][method]["additive"][None][split] = {
                        "trues": test_obs,
                        "pred": train_obs,
                        "train_trues": train_data,
                    }
...
def plot_metric_mutation_comparison(
    datasets: List[str],
    algos: List[str],
    metrics: str,
    reps: List[str],
    ...
    results_dict = load_mutation_results_with_baseline(
        datasets,
        algos,
        metrics,
        reps,
...
def plot_mutation_comparison_matrix(
    datasets: List[str],
    algos: List[str],
    metrics: str,
    reps: List[str],
...
def plot_metric_functional_threshold_comparison(
    datasets: List[str],
    algos: List[str],
    metrics: List[str],
    reps: List[str],
...
def plot_metric_augmentation_comparison(
    ...
    ref_results_dict = {}
    if reference_bars:
        ref_results_dict = get_mlflow_results(
            datasets=datasets,
            algos=algos,
            reps=reps,
            metrics=metrics,
    ...
    barplot_metric_augmentation_comparison(
        metric_values=results_dict,
        cvtype=train_test_splitter,
        augmentation=augmentation,
        metric=MSE,
        dim=dim,
        dim_reduction=dim_reduction,
        reference_values=ref_results_dict,
    )
...
if __name__ == "__main__":
    ...
    plot_metric_comparison_bar_splitting(
        datasets=["1FQG", "UBQT"],
        reps=[ESM],
        metrics=metrics,
        train_test_splitter=[RandomSplitter("1FQG"), PositionSplitter("1FQG")]
        + fractional_splitters
        + [OptimizationSplitter("1FQG")],
        algos=[GPonRealSpace(kernel_factory=lambda: Matern52()).get_name()],
        color_by="task",
        x_axis="task",
        seeds=seeds,
    ...
    plot_mutation_comparison_matrix(
        datasets=["TOXI"],
        algos=[GPonRealSpace(kernel_factory=lambda: Matern52()).get_name()],
        metrics=[SPEARMAN_RHO, "base_MSE"],
        reps=[ONE_HOT, EVE, ESM],
        train_test_splitter=[
            BioSplitter("TOXI", 1, 1),
            BioSplitter("TOXI", 1, 2),
            BioSplitter("TOXI", 2, 2),
            BioSplitter("TOXI", 2, 3),
            BioSplitter("TOXI", 3, 3),
            BioSplitter("TOXI", 3, 4),
        ],
        dimension=None,
        dim_reduction=None,
        cached_results=True,
    ...
    for rep in [ESM, ONE_HOT, EVE]:
        plot_metric_comparison_bar_param_delta(
            datasets=[
                "1FQG",
                "UBQT",
                "TIMB",
    ...
    for ablation_dataset in ["TIMB", "1FQG"]:
        ablation_protocols_random_cv = [
            RandomSplitterFactory(ablation_dataset, k=i)[0] for i in [2, 3, 5, 7, 15]
        ] + [
            PositionalSplitterFactory(ablation_dataset, positions=p)[0]
            for p in [5, 10, 20, 25]
        ]
