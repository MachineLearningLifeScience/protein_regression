...
from util.mlflow.constants import AUGMENTATION, DATASET, LINEAR, METHOD, MSE, SPEARMAN_RHO, MLL
...
def plot_cumulative_comparison(datasets: List[str], 
                            ...
                            dim_reduction=None):
    results_dict = {}
    for frac, splitter in zip(testing_fractions[:20], train_test_splitters[:20]):
...
if __name__ == "__main__":
    ...
    algos = [GPonRealSpace(kernel_factory= lambda: SquaredExponential()).get_name(), GPonRealSpace(kernel_factory= lambda: Matern52()).get_name(),
             RandomForest().get_name()]
    metrics = [MLL, MSE, SPEARMAN_RHO] # MSE
    representations = [TRANSFORMER] # # SPECIAL CASE [UBQT, BLAT]: VAE+"_clusterval" 
    dim = None
